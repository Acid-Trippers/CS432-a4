...
function persistDeveloperModeSetting(enabled) {
  ...
  try {
    localStorage.setItem(DEV_MODE_STORAGE_KEY, developerModeEnabled ? "on" : "off");
  } catch {
...
function renderDashboardStatsBundle(status, stats, pipelineStats, developerStats = null) {
  ...
  if (developerModeEnabled) {
    const queryMetrics = developerStats?.latest_query?.metrics || {};
    const totalMs = Number(queryMetrics.total_time_ms || 0).toFixed(3);
    const readThroughput = Number(queryMetrics.throughput_records_per_sec || 0).toFixed(3);
    const reportsCount = formatInteger(developerStats?.performance_reports?.count || 0);
    ...
    setKpiCardContent(
      ...
      reportsCount,
      "Saved performance report files",
    );
  }
}
...
async function refreshDashboardStats() {
  ...
  try {
    ...
    renderDashboardStatsBundle(status, stats, pipelineStats, developerStats);
...
function renderDeveloperQueryMetrics(latestQuery) {
  ...
  if (!latestQuery || typeof latestQuery !== "object") {
    body.innerHTML = '<tr><td class="meta-text">No query metrics available.</td></tr>';
    return;
  }
...
async function refreshDeveloperMetricsPage() {
  ...
  try {
    ...
    if (refreshed) refreshed.textContent = `Last refreshed: ${formatClockTime(new Date())}`;
  } catch (error) {
    setFeedback(feedback, String(error.message || error), true);
